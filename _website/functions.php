...
function glists($in, $order = "position ASC", $limit = 20, $popular = false){
	$pop = ($popular==true) ? ' AND `catalogs`.`popu`=1' : '';
	$sql = "SELECT 
		(
			SELECT `pages`.`title` 
			FROM 
			...
			`pages`.`deleted`=0 AND 
			`pages`.`menutype`=`catalogs`.`menuid`
		) AS categorytitle, 
		`catalogs`.* 
		FROM 
		`catalogs` 
		...
		`catalogs`.`menuid` IN(".implode($in, ",").") AND 
		`catalogs`.`deleted` = 0 AND 
		`catalogs`.`visibility` = 1".$pop." 
		...
		$out.= '					
			<div class="col-sm-2">
				<a href="'.$link.'" class="Item">
					...
					<div class="Info">
						<div class="CompanyLogo"><img src="'.$item['image1'].'"/></div>
						<div class="Category">'.$item['categorytitle'].'</div>
						<div class="Title">' . $item['title'] . '</div>
						<li class="Address">' . $item['address'] . '</li>
						<li class="Time">'.$item['MonFri'].'</li>
					</div>
...
function gblog(){
	$sql = "SELECT * FROM " . c("table.pages") . " WHERE language='".l()."' AND menuid=22 AND deleted=0 ORDER BY postdate DESC LIMIT 20";
    $items = db_fetch_all($sql);
    $out = "";
    foreach($items as $item){
		$link = href($item['id']);	
		$out.= '					
			<div class="col-sm-2">
				<a href="'.$link.'" class="Item">
					<div class="Image" style="height:210px;"><img src="'.$item['image1'].'"/></div>
					<div class="Info" style="height: auto;">
						<!--<div class="CompanyLogo"><img src="'.$item['image2'].'"/></div>-->
						<div class="Title">' . $item['title'] . '</div>
						<!--<li class="Time">'.convert_date($item['postdate']).'</li>-->
					</div>
				</a>
...
function categories_menu_home_search() {
    ...
    for ($idx = 0, $out = NULL, $num = count($sections); $idx < $num; $idx++)
    {
		    $sql_in = "SELECT id, title, idx, image2, redirectlink, menutitle, menutype, category, masterid,slug FROM " . c("table.pages") . " WHERE language = '" . l() . "' AND menuid = 1 AND deleted = 0 AND masterid = " . $sections[$idx]['id'] . " AND visibility = 1 ORDER BY position;";
	    	$sections_in = db_fetch_all($sql_in);
	    	$cnt_sections_in = count($sections_in);
			if($cnt_sections_in > 0) {
				for ($idx_in = 0, $num_in = count($sections_in); $idx_in < $num_in; $idx_in++)
	    		{
			        $link_in = (($sections_in[$idx_in]['redirectlink'] == '') || ($sections_in[$idx_in]['redirectlink'] == 'NULL')) ? href($sections_in[$idx_in]['id']) : $sections_in[$idx_in]['redirectlink'];
					$out .= '
						<li data-id="'.$sections_in[$idx_in]['menutype'].'" data-title="'.htmlentities($sections_in[$idx_in]['menutitle']).'" data-slug="'.htmlentities($sections_in[$idx_in]['slug']).'"><a href="javascript:;"><i class="fa ' . $sections_in[$idx_in]['image2'] . '"></i>' . $sections_in[$idx_in]['menutitle'] . '</a></li>					
							'."\n";
				}
			}
    }
...
function categories_menu_home() {
    ...
    for ($idx = 0, $out = NULL, $num = count($sections); $idx < $num; $idx++)
    {
		    $sql_in = "SELECT id, title, idx, image2, redirectlink, menutitle, category, masterid, slug FROM " . c("table.pages") . " WHERE language = '" . l() . "' AND menuid = 1 AND deleted = 0 AND masterid = " . $sections[$idx]['id'] . " AND visibility = 1 ORDER BY position;";
	    	$sections_in = db_fetch_all($sql_in);
	    	$cnt_sections_in = count($sections_in);
			if($cnt_sections_in > 0) {
				for ($idx_in = 0, $num_in = count($sections_in); $idx_in < $num_in; $idx_in++)
	    		{
			        $link_in = (($sections_in[$idx_in]['redirectlink'] == '') || ($sections_in[$idx_in]['redirectlink'] == 'NULL')) ? href($sections_in[$idx_in]['id']) : $sections_in[$idx_in]['redirectlink'];
			        $real_url = explode("/", $_SERVER['REQUEST_URI']);
			        $slug = @$real_url[2]."/".@$real_url[3];
			        $active = ($sections_in[$idx_in]["slug"]==$slug) ? " g-active" : "";
					$out .= '
						<a href="' .$link_in.'" class="Item g-category-items'.$active.'">
...
function g_banners(){
	$select = db_fetch_all("SELECT `menutype` FROM " . c("table.pages") . " WHERE  language = '" . l() . "' AND deleted=0 AND id IN (590, 591, 594, 595, 592, 593) AND visibility = 1 order by position ASC" );
	$ids = array();
	foreach ($select as $item):
		$ids[] = $item["menutype"];
	endforeach;

	$banners = db_fetch_all("SELECT `id`,`menuid`,`title`,`link`,`image1`,`image2`,`htmlstring` FROM " . c("table.galleries") . " WHERE  language = '" . l() . "' AND deleted=0 AND menuid IN(".implode(",", $ids).") AND visibility = 1 order by position ASC" );
	$out = array();
	foreach ($banners as $item):
		$out["banner".$item["menuid"]][] = $item;
	endforeach;
