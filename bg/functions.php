...
{
//    include_once DIR . 'bg/firephp/FirePHP.class.php';
...
function template($file, $variables = array())
{
    ...
    $templatefiles = array(
        ...
        DIR . 'bg/templates/' . $file . '.php',
        DIR . 'bg/templates/default' . $file . '.php'
    );
...
function site_template($file, $variables = array())
{
    $files = array(
        ...
        'bg/templates/' . $file . '.php',
        'bg/templates/default' . $file . '.php'
    );
    foreach ($files as $template)
    {
        if (!file_exists($template))
        ...
        if($item["category"]==16) {
            ...
            $product = db_fetch("
                ...
                (
                    ...
                    `pages`
                    WHERE 
                    `pages`.`menutype`=`catalogs`.`menuid` AND 
                ...
                `catalogs` 
                WHERE 
                `catalogs`.`id` = {$last_segment} AND 
                `catalogs`.`deleted`=0 AND 
                `catalogs`.`language`='".l()."' LIMIT 1");
...
function text_limit($string, $limit=130, $break=".", $end="")
{
    $nbsp = array("&nbsp;");
    $del_nbsp = array(" ");
    $string = str_replace($nbsp, $del_nbsp, $string);
