...
//    $undeletable = c('section.undeletable');
    ...
    foreach ($results AS $result):
        ...
        $is_first = ($count == 1);
        $is_last = ($count >= $total);
        $arrow_up = $is_first ? '<img src="bg/img/none.png" width="9" height="9" style="float:left;" />' : '<a href="javascript:pos(\'up\',\''.$result['id'].'\')"><img src="bg/img/buttons/icon_arrow_up_' . ($result['level'] > 4 ? 1 : $result["level"] - 1) . '.png" class="star" title="' .  a('tt.moveup') . '" width="9" height="9" /></a>';
        $arrow_down = $is_last ? '<img src="bg/img/none.png" width="9" height="9" style="float:left;" />' : '<a href="javascript:pos(\'down\',\''.$result['id'].'\')"><img src="bg/img/buttons/icon_arrow_down_' . ($result['level'] > 4 ? 1 : $result["level"] - 1) . '.png" class="star" title="' .  a('tt.movedn') . '" width="9" height="9" /></a>';
		...
		$pad .= $lnks . '<img src="bg/img/' . $collapse . '.png" width="16" height="16" style="margin:0 0 0 ' . ((($result['level'] - 1) * 25) + 5) . 'px;" >' . $lnke;
		$pad .= '<img src="bg/img/buttons/' . $bn . '.png" style="margin:0 10px 0 0;" >';
		$spad .= '<img src="bg/img/none.png" style="margin:0 5px 0 ' . ((($result['level'] - 1) * 25) + 5) . 'px;" >&nbsp;';
		$ch = ($result["visibility"]==1) ? '' : 'un';
?>

                    <div class="<?php echo $class;?> fix" id="div<?php echo $result["id"];?>" >
						<div class="check">
							<a href="javascript:chclick(<?php echo $result['id'];?>);"><img src="bg/img/buttons/icon_<?php echo $ch;?>visible.png" class="star" title="<?php echo a('tt.visibility');?>" id="img_<?php echo $result['id'];?>" style="width:9px;height:9px;" /></a>
...
<?php echo mb_substr(href($result["id"]), 0,84,'UTF-8') . ((mb_strlen(href($result["id"]),'UTF-8')>84) ? '...' : '');?>
                        ...
                        <div class="action fix">
                        	<a href="<?php echo href($result['id']) ?>" target="_blank"><img src="bg/img/buttons/icon_preview.png" class="star" title="<?php echo a('ql.preview');?>" /></a>
                            <a href="<?php echo ahref(array($route[0], 'edit', $result["id"]));?>"><img src="bg/img/buttons/icon_edit.png" class="star" title="<?php echo a('ql.edit');?>" /></a>
<?php
...
?>
							<a href="<?php echo ahref(array($menulink1, 'show', $result["menutype"])) ?>"><img src="bg/img/buttons/icon_files<?php echo ($page_count['cnt']==0)	? '' : '_r';?>.png" class="star" title="<?php echo a('ql.editcontent');?>" /></a>
<?php
		}
	}
?>
...
?>
                            <a href="<?php echo ahref(array('files', '', $result["id"]));?>"><img src="bg/img/buttons/icon_attach<?php echo ($att_count['cnt']==0)	? '' : '_r';?>.png" class="star" title="<?php echo a('ql.attachments');?>" /></a>
			<?php endif; ?>
<?php endif; ?>
                            <a href="javascript:addr('Page Address:', '<?php echo href($result['id']) ?>');"><img src="bg/img/buttons/icon_link.png" class="star" title="<?php echo a('ql.link');?>" /></a>
<?php if($route[0] =='sitemap') : ?>
							<a href="<?php echo ($result["level"]<3 && $result["id"]!=1) ? ahref(array($route[0], 'add', $result['menuid']), array('id' => $result["id"])) : 'javascript:;';?>"><img src="bg/img/buttons/<?php echo ($result["level"]<3 && $result["id"]!=1) ? 'icon_add' : 'icon_no_add' ?>.png" class="star" title="<?php echo a('ql.addsubpage');?>" /></a>
<?php endif; ?>
<?php
	...
	if($undeletable == 0) {
?>
                            <a href="javascript:del(<?php echo $result['id'];?>, '<?php echo $result['title'];?>');"><img src="bg/img/buttons/icon_delete.png" class="star" title="<?php echo a('ql.delete');?>" /></a>
<?php } else { ?>
							<a href="javascript:"><img src="bg/img/buttons/icon_delete_inactive.png" class="star" title="" style="width:15px;height:15px;" /></a>
<?php } ?>
                        </div>
...
?>
		<div id="title" class="fix">
			<div class="icon"><img src="bg/img/table.png" width="16" height="16" alt="" /></div>
...
?>
					<ul id="page" class="fix left" style="width:900px;">
						<li><a href="<?php echo ahref($route, $params) . '&start=0'?>"><img src="bg/img/prev2.png" width="5" height="5" alt=""  class="arrow" /></a></li>
						<li><a href="<?php echo ahref($route, $params) . '&start=' . $prev;?>"><img src="bg/img/prev.png" width="5" height="5" alt=""  class="arrow" /></a></li>
<?php
...
<?php } ?>
						<li><a href="<?php echo ahref($route, $params) . '&start=' . $next;?>"><img src="bg/img/next.png" width="5" height="5" alt="" class="arrow" /></a></li>
						<li><a href="<?php echo ahref($route, $params) . '&start=' . $last;?>"><img src="bg/img/next2.png" width="5" height="5" alt="" class="arrow" /></a></li>
					</ul>
<?php } ?>
<?php if($route[0]=='sitemap') { ?>
