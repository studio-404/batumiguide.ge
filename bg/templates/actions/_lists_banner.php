...
function _build_lists($parent, $order_by, $start = -1, $per_page)
{
    ...
    foreach ($results AS $result):
        $is_first = ($count == 1);
        $is_last = ($count >= $total);
        $arrow_up = $is_first ? '<img src="bg/img/none.png" width="10" height="7" style="float:left;" />' : '<a href="' . ahref($action, 'move', array('where' => 'up', 'idx' => $result['idx'], 'menu' => $result['menuid'])) . '"><img src="bg/img/buttons/icon_arrow_up_' . ($result['level'] == 1 ? 1 : 2) . '.png" class="star" title="' .  a('tt.moveup') . '" width="10" height="7" /></a>';
        $arrow_down = $is_last ? '<img src="bg/img/none.png" width="10" height="7" style="float:left;" />' : '<a href="' . ahref($action, 'move', array('where' => 'down', 'idx' => $result['idx'], 'menu' => $result['menuid'])) . '"><img src="bg/img/buttons/icon_arrow_down_' . ($result['level'] == 1 ? 1 : 2) . '.png" class="star" title="' .  a('tt.movedn') . '" width="10" height="7" /></a>';
		$arrows = ($order_by == 'position') ? $arrow_up . $arrow_down : '<img src="bg/img/table.png" width="16" height="16" alt="" />';
...
?>
					<div class="<?php echo $class;?> fix" id="div<?php echo $result["id"];?>" >
						<div class="check" style="float:left;">
							<a href="javascript:chclick(<?php echo $result['idx'];?>);"><img src="bg/img/buttons/icon_<?php echo $ch;?>visible.png" class="star" title="<?php echo a('tt.visibility');?>" id="img_<?php echo $result['idx'];?>" style="width:9px;height:9px;" /></a>
                        ...
                        <div class="action fix" style="width:120px;">
                            <a href="<?php echo ahref($action, 'edit', array('menu'=>$result['menuid'], 'id'=>$result["id"], 'idx'=>$result["idx"]));?>"><img src="bg/img/buttons/icon_edit.png" class="star" title="<?php echo a('ql.edit');?>" /></a>
...
?>
					<ul id="page" class="fix left">
						<li><a href="<?php echo ahref($action, $subaction, $params) . '&start=' . $first;?>"><img src="bg/img/prev2.png" width="5" height="5" alt=""  class="arrow" /></a></li>
						<li><a href="<?php echo ahref($action, $subaction, $params) . '&start=' . $prev;?>"><img src="bg/img/prev.png" width="5" height="5" alt=""  class="arrow" /></a></li>
<?php
...
?>
						<li><a href="<?php echo ahref($action, $subaction, $params) . '&start=' . $next;?>"><img src="bg/img/next.png" width="5" height="5" alt="" class="arrow" /></a></li>
						<li><a href="<?php echo ahref($action, $subaction, $params) . '&start=' . $last;?>"><img src="bg/img/next2.png" width="5" height="5" alt="" class="arrow" /></a></li>
					</ul>
<?php } ?>
					<a href="<?php echo ahref($action, 'add', array('menu'=>$params['menu'], 'level'=>0));?>" class="button br" style="float: right"><?php echo a('add');?></a>
				</div>					
...
function chclick(idx) {
	...
	$.post("<?php echo ahref($action, 'visibility');?>&idx=" + idx + "&visibility=" + active + "&menu=" + <?php echo $params["menu"];?>, function() {
		if(active=='true') 
	        $('#img_'+idx).attr("src","bg/img/buttons/icon_visible.png"); 
		else
	        $('#img_'+idx).attr("src","bg/img/buttons/icon_unvisible.png"); 
		$('#vis_'+idx).val(active);
	});
};	
