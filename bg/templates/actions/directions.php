...
<?php
	...
	foreach($files as $file) :
        $is_first = ($count == 1);
        $is_last = ($count >= $total);
        $arrow_up = $is_first ? '<img src="bg/img/none.png" width="10" height="7" />' : '<a href="' . ahref('files', 'move', array('where' => 'up', 'menu' => $menu, 'id' => $id, 'idx'=> $idx, 'pos'=> $file['id'])) . '"><img src="bg/img/buttons/icon_arrow_up.png" class="star" title="' .  a('tt.moveup') . '" width="10" height="7" /></a>';
        $arrow_down = $is_last ? '<img src="bg/img/none.png" width="10" height="7" />' : '<a href="' . ahref('files', 'move', array('where' => 'down', 'menu' => $menu, 'id' => $id, 'idx'=> $idx, 'pos'=> $file['id'])) . '"><img src="bg/img/buttons/icon_arrow_down.png" class="star" title="' .  a('tt.movedn') . '" width="10" height="7" /></a>';
...
?>
					<div class="<?php echo $class;?> fix" id="div<?php echo $file["id"];?>">
						<ul class="action fix">
                        	<a href="<?php echo href(get('id',1)) ?>" target="_blank"><img src="bg/img/buttons/icon_preview.png" class="star" title="<?php echo a('ql.preview');?>" /></a>
                            <a href="javascript:del(<?php echo $file['id'];?>, '<?php echo $file['title'];?>');"><img src="bg/img/buttons/icon_delete.png" class="star" title="<?php echo a('ql.delete');?>" /></a>
						</ul>
						<div class="icon"></div>
